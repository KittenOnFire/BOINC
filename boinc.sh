...
sudo apt-get update && echo "Update du syst�me effectu�e" # Update et upgrade de l'OS

sudo apt-get install boinc-client && echo "BOINC install�" # Installation du client BOINC
...
read manager
echo -e \t\e[1;33;40m"Quelle est l'adresse mail utilis�e avec ce manager ?" \e[0m 1;33m
read mail
...
sudo boinccmd --set_network_mode always && echo -e "BOINC a maintenant toujours acc�s � Internet" # Toujours �tre reli� au r�seau

echo -e \t\e[1;33;40m"Adresse ip locale de l'ordinateur h�te (ayant acc�s � celui-ci). Exemple : 192.168.1.10" \e[0m 1;33m
read ip_master
sudo echo $ip_master > /etc/boinc-client/remote_hosts.cfg && echo "IP de l'ordinateur h�te inscrite dans le fichier remote_hosts.cfg" # Inscrire l'IP de l'ordi h�te

echo -e \t\e[1;33;40m"Donnez un nom � cette machine" \e[0m 1;33m
read nom
...
sudo /etc/init.d/boinc-client restart && echo " Client BOINC red�marr�. Bon crunch !"
